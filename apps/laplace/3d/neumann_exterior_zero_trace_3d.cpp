...
#include "src/support/trace/exterior_only_cubic_normal_restrict_3d.hpp"
...
SupportPathMode3D selected_support_path_mode()
{
    ...
    if (raw == nullptr || std::string(raw).empty()
        ...
        return SupportPathMode3D::Legacy;
...
{
    return selected_support_path_mode() == SupportPathMode3D::NativeCertified;
}

bool selected_closest_point_support_path()
{
    return selected_support_path_mode() == SupportPathMode3D::ClosestPoint;
}

const char* selected_support_path_name()
{
...
bool selected_preprocess_only()
{
    const char* raw = std::getenv("KFBIM_3D_PREPROCESS_ONLY");
    if (raw == nullptr || *raw == '\0' || std::string(raw) == "0")
        return false;
    if (std::string(raw) == "1")
        return true;
    throw std::invalid_argument("KFBIM_3D_PREPROCESS_ONLY must be 0 or 1");
}
...
public:
    struct ClosestPointDiagnostics {
        std::size_t queries = 0;
        std::size_t cache_hits = 0;
        std::size_t corrections = 0;
        std::size_t same_side_nodes = 0;
        std::size_t legacy_fallbacks = 0;
        std::array<std::size_t, 8> rejection_counts{};
        std::size_t boxes_visited = 0;
        std::size_t maximum_boxes_per_query = 0;
        double query_seconds = 0.0;
        double legacy_query_seconds = 0.0;
        double maximum_distance_gap = 0.0;
    };
    ...
    PanelCenterHarmonicJetKFBI3D(const CartesianGrid3D& grid,
                                 ...
                                 bool use_closest_point_extension = false)
        : grid_(grid)
        ...
        , closest_point_extension_(use_closest_point_extension)
        , h_(grid.spacing()[0])
    ...
    {
        if (closest_point_extension_) {
            if (native_endpoint_paths_
                || (!build_q27_cover3_restrict && !build_q64_cover4_restrict)
                || build_shared_quadratic_restrict) {
                throw std::invalid_argument(
                    "closest-point extension requires a Q27/Q64 cover route and excludes other support-path modes");
            }
            closest_point_index_ =
                std::make_unique<geometry3d::NurbsSurfaceClosestPointIndex3D>(
                    native_surface_.geometry_model());
            closest_point_options_.distance_tolerance = 1.0e-9 * h_;
            closest_point_options_.localization_tolerance = 1.0e-6 * h_;
        }
        if (native_endpoint_paths_) {
        ...
        if (build_shared_quadratic_restrict
            ...
            closest_point_cache_.clear();
            closest_point_index_.reset();
        }
...
private:
    ...
    bool try_build_closest_point_correction(
        int center,
        int stencil_node,
        int node,
        bool node_inside,
        ...
        std::vector<SharedQuadraticGridlineCorrection3D>& result) const
    {
        if (!closest_point_extension_)
            return false;
        auto cached_projection = closest_point_cache_.find(node);
        if (cached_projection == closest_point_cache_.end()) {
            const auto query_start = std::chrono::steady_clock::now();
            auto projection = closest_point_index_->query(
                node_point, closest_point_options_, closest_point_workspace_);
            closest_point_diagnostics_.query_seconds +=
                std::chrono::duration<double>(
                    std::chrono::steady_clock::now() - query_start).count();
            ++closest_point_diagnostics_.queries;
            closest_point_diagnostics_.boxes_visited +=
                projection.stats.boxes_visited;
            closest_point_diagnostics_.maximum_boxes_per_query = std::max(
                closest_point_diagnostics_.maximum_boxes_per_query,
                static_cast<std::size_t>(projection.stats.boxes_visited));
            cached_projection = closest_point_cache_.emplace(
                node, std::move(projection)).first;
        } else {
            ++closest_point_diagnostics_.cache_hits;
        }
        const SurfaceDof& target = cloud_.dofs[static_cast<std::size_t>(center)];
        const auto& projection = cached_projection->second;
        const auto anchor = app3d::select_closest_point_trace_anchor_3d(
            native_surface_, projection, node_point, target.point,
            target.patch_id, target.normal, 3.0 * h_);
        if (!anchor.accepted()) {
            ++closest_point_diagnostics_.legacy_fallbacks;
            const auto reason = static_cast<std::size_t>(anchor.status);
            if (reason < closest_point_diagnostics_.rejection_counts.size())
                ++closest_point_diagnostics_.rejection_counts[reason];
            return false;
        }
        // Equal binary labels alone do not establish a common local branch:
        // a support path across a thin wall may cross twice. Only skip the
        // jump after the closest anchor passes the local-sheet checks.
        if (node_inside == desired_inside) {
            ++closest_point_diagnostics_.same_side_nodes;
            return true;
        }
        const auto& owner = anchor.owner;
        const std::tuple<int, double, double> key{
            owner.nurbs_patch_index,
            owner.nurbs_parameter.x(), owner.nurbs_parameter.y()};
        auto cached_plan = all_event_exact_cauchy_cache_.find(key);
        if (cached_plan == all_event_exact_cauchy_cache_.end()) {
            const int anchor_center = surface_dof_for_crossing(
                owner, owner.crossing_point);
            cached_plan = all_event_exact_cauchy_cache_.emplace(
                key, build_direct_crossing_plan(
                    owner, owner.crossing_point, anchor_center)).first;
        }
        const auto& plan = cached_plan->second;
        auto correction = compose_shared_quadratic_correction(
            stencil_node, node, plan, anchor.distance / h_);
        // The local Cauchy field is [u] = u_inside - u_outside.  Both
        // directions use the same anchor and polynomial, with opposite signs.
        correction.continuation_sign =
            static_cast<int>(desired_inside) - static_cast<int>(node_inside);
        correction.direct_crossing_owner = owner;
        correction.direct_grid_node = node;
        result.push_back(std::move(correction));
        ++closest_point_diagnostics_.corrections;
        closest_point_diagnostics_.maximum_distance_gap = std::max(
            closest_point_diagnostics_.maximum_distance_gap,
            projection.distance_upper - projection.distance_lower);
        shared_quadratic_diagnostics_.cauchy_condition_max = std::max(
            shared_quadratic_diagnostics_.cauchy_condition_max, plan.condition);
        shared_quadratic_diagnostics_.gridline_distance_max_over_h = std::max(
            shared_quadratic_diagnostics_.gridline_distance_max_over_h,
            anchor.distance / h_);
        return true;
    }
        ...
        const app3d::G1PatchTopology3D& topology) const
    {
        ...
        for (int stencil_node = 0;
            ...
            if (try_build_closest_point_correction(
                    center, stencil_node, node, node_inside, desired_inside,
                    node_point, result))
                continue;
            ...
            if (cached_intersection == all_event_intersection_cache_.end()) {
                ...
                cached_intersection = all_event_intersection_cache_.emplace(
                    ...
                    intersector.intersect_segment(node_point, certified_end))
                                          .first;
                if (closest_point_extension_) {
                    closest_point_diagnostics_.legacy_query_seconds +=
                        std::chrono::duration<double>(
                            std::chrono::steady_clock::now() - query_start).count();
                }
            }
            ...
            if (endpoint_partition.kind
                ...
                if (closest_point_extension_) {
                    const auto& projection = closest_point_cache_.at(node);
                    const auto anchor = app3d::select_closest_point_trace_anchor_3d(
                        native_surface_, projection, node_point, dof.point,
                        dof.patch_id, dof.normal, 3.0 * h_);
                    message << " closest_anchor_status="
                            << app3d::closest_point_trace_anchor_status_name_3d(anchor.status)
                            << " closest_patch=" << projection.patch_index
                            << " closest_distance=" << projection.distance_upper
                            << " closest_localized=" << projection.localized
                            << " closest_radius=" << projection.localization_radius_upper
                            << " closest_boxes=" << projection.stats.boxes_visited;
                }
                throw std::runtime_error(message.str());
            }
        ...
        app3d::TensorProductCoverKind3D kind)
    {
        ...
        for (int center = 0; center < surface_size(); ++center) {
            ...
            if (closest_point_extension_
                && ((center + 1) % 128 == 0 || center + 1 == surface_size())) {
                std::cout << "[closest_point] centers=" << center + 1
                          << '/' << surface_size()
                          << " queries=" << closest_point_diagnostics_.queries
                          << " corrections=" << closest_point_diagnostics_.corrections
                          << " legacy_fallbacks=" << closest_point_diagnostics_.legacy_fallbacks
                          << " query_seconds=" << closest_point_diagnostics_.query_seconds
                          << '\n';
            }
        }
    ...
    mutable geometry3d::NurbsSurfaceClosestPointWorkspace3D
        closest_point_workspace_;
    mutable std::map<int, geometry3d::NurbsSurfaceClosestPointResult3D>
        closest_point_cache_;
    mutable ClosestPointDiagnostics closest_point_diagnostics_;
...
private:
    static std::unique_ptr<app3d::ReducedTraceProjection3D> make_projection(
        const app3d::NativeNurbsDensitySpace3D& density,
    ...
    std::unique_ptr<app3d::ReducedTraceProjection3D> projection_;
                                   ...
                                   SolveSelection3D solve_selection)
{
    ...
    const auto trace_setup_start = std::chrono::steady_clock::now();
    PanelCenterHarmonicJetKFBI3D harmonic_pipeline(
        ...
        selected_closest_point_support_path());
    const double trace_setup_seconds = std::chrono::duration<double>(
        std::chrono::steady_clock::now() - trace_setup_start).count();
    ...
    const auto write_preprocessing_result = [&]() {
        ...
        json << std::setprecision(17)
             ...
             << "  \"N\": " << N << ",\n"
             << "  \"support_path\": \"" << selected_support_path_name() << "\",\n"
             << "  \"trace_density_preprocess_complete\": true,\n"
             << "  \"snapshot_stage\": \"before_solve_specific_topology_and_gmres\",\n"
             << "  \"completed_scope\": \"" << completed_scope << "\",\n"
             << "  \"gmres_started\": false,\n"
             << "  \"setup_seconds\": " << elapsed << ",\n"
             << "  \"trace_setup_seconds\": " << trace_setup_seconds << ",\n"
             << "  \"surface_dofs\": " << result.surface_dofs << ",\n"
             << "  \"closest_point_queries\": " << d.queries << ",\n"
             << "  \"closest_point_cache_hits\": " << d.cache_hits << ",\n"
             << "  \"closest_point_corrections\": " << d.corrections << ",\n"
             << "  \"same_side_nodes\": " << d.same_side_nodes << ",\n"
             << "  \"legacy_fallbacks\": " << d.legacy_fallbacks << ",\n"
             << "  \"legacy_segment_queries\": " << result.shared_quadratic_restrict.segment_queries << ",\n"
             << "  \"closest_point_boxes\": " << d.boxes_visited << ",\n"
             << "  \"maximum_boxes_per_query\": " << d.maximum_boxes_per_query << ",\n"
             << "  \"closest_point_query_seconds\": " << d.query_seconds << ",\n"
             << "  \"legacy_query_seconds\": " << d.legacy_query_seconds << ",\n"
             << "  \"accepted_maximum_distance_gap\": " << d.maximum_distance_gap << ",\n"
             << "  \"constant_exterior_value_error\": " << result.harmonic_constant_exterior_trace_linf << ",\n"
             << "  \"constant_interior_value_error\": " << result.harmonic_constant_interior_trace_linf << ",\n"
             << "  \"constant_bulk_error\": " << result.harmonic_constant_bulk_linf << ",\n"
             << "  \"fallback_reason_order\": [\"accepted\",\"invalid_input\",\"unresolved_projection\",\"ambiguous_projection\",\"incompatible_sheet\",\"feature_contact\",\"non_normal_projection\",\"too_far\"],\n"
             << "  \"fallback_reason_counts\": [";
        for (std::size_t i = 0; i < d.rejection_counts.size(); ++i) {
            if (i != 0) json << ',';
            json << d.rejection_counts[i];
        }
        json << "]\n}\n";
        if (!json)
            throw std::runtime_error("failed to write preprocessing diagnostics");
        if (selected_preprocess_only()) {
            std::cout << "KFBIM_PREPROCESS_COMPLETE geometry=" << geometry.name
                      << " N=" << N << " seconds=" << elapsed
...
void print_usage(const char* executable)
{
    ...
    std::cout
        ...
        << "  and density-binding setup, before solve-specific topology/GMRES.\n"
...
int main(int argc, char** argv)
{
    try {
        ...
        if (selected_preprocess_only() && (rigid_study || restrict_probe))
            throw std::invalid_argument(
                "KFBIM_3D_PREPROCESS_ONLY requires a named geometry run");
        ...
        if (selected_preprocess_only()) {
            std::cout << "Preprocessing output: " << output_dir.string() << '\n';
            return 0;
        }
